...
class CredentialManager:
    ...
    def set_credential(
        self, key: str, value: str, username: Optional[str] = None
    ) -> None:
    ...
    def get_ig_credentials(self) -> tuple[str, str]:
        ...
        if not password:
            raise ValueError(
                f"IG_PASSWORD not found in keychain or .env for user '{username}'"
            )
    ...
    def interactive_setup(self) -> None:
        ...
        wa_phone = input(
            "WhatsApp phone (international format, e.g., 491701234567): "
        ).strip()
...
def load_credentials_safely() -> dict[str, str]:
    ...
    wa_report_contact = (
        manager.get_credential("WA_REPORT_CONTACT_NAME") or wa_content_contact
    )
    ...
    if not wa_content_contact:
        raise ValueError(
            "WA_CONTENT_CONTACT_NAME not found. Run: python -m src.credentials"
        )
